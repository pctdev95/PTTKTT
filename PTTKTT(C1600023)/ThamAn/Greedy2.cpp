...
DoVat * ReadFromFile(float *W, int *n){
	...
	int i=0;
	while (!feof(f)){
 		fscanf(f, "%f%f%f", &dsdv[i].TL, &dsdv[i].GT);
...
void Greedy(DoVat *dsdv,int n, float W){
	int i;
	for(i = 0; i < n; i++) {
		if((W - dsdv[i].TL) > 0){ //Neu Tong trong luong - trong luong do vat > 0 => (Balo con cho trong. Co the them do vat vao balo)
			dsdv[i].PA = 1; //PA do vat i  = 1
...
int main(){
	...
	Greedy(dsdv,n,W);			//Dung thuat toan tham an de lua chon ra phuong an
